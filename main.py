...
def draw_all_balls():
    for ball in balls:
        pygame.draw.circle(
            window, ball["color"], (ball["x"], ball["y"]), ball["radius"]
        )
...
def simulate_physics():
    ...
    for ball in balls:
        ...
        if is_make_ball_color_velocity:
            color = min(
                255, ((abs(ball["vector"]["x"] + ball["vector"]["y"]) / 2) * 0.1) * 255
            )
        ...
        for j, ball2 in enumerate(balls):
            if i != j:
                ...
                if dist < ball1["radius"] + ball2["radius"]:
                    ...
                    v1n = (
                        ball1["vector"]["x"] * dx_norm + ball1["vector"]["y"] * dy_norm
                    )
                    v2n = (
                        ball2["vector"]["x"] * dx_norm + ball2["vector"]["y"] * dy_norm
                    )
...
def draw_floor():
    pygame.draw.rect(
        window, (200, 200, 200), (0, window_size[1] - 25, window_size[0], 25)
    )
...
def check_colision(obj1, obj2) -> bool:
    return (
        ...
        and obj1["x"] + obj1["radius"] > obj2["x"]
        and obj1["y"] < obj2["y"] + obj2["radius"]
        and obj1["y"] + obj1["radius"] > obj2["y"]
    )
...
def draw_help_menu():
    ...
    small_text = font = pygame.font.Font(None, 24).render(
        "Press ESC to close this menu", True, (255, 255, 255)
    )
...
while running:
    ...
    if pygame.mouse.get_pressed()[0] and not (
        help_menu_pressed or info_menu_pressed
    ):  # Left mouse button
        mouse_x, mouse_y = pygame.mouse.get_pos()
        if len(balls) == 0:
            create_ball(mouse_x, mouse_y, 0, 0, 20)
        if (
            distance(balls[-1]["x"], balls[-1]["y"], mouse_x, mouse_y) > 35
            and len(balls) < 200
        ):
    ...
    # Catapult ball from its original position
        ...
        and not (help_menu_pressed or info_menu_pressed)
    ):  # Right mouse button
        mouse_x, mouse_y = pygame.mouse.get_pos()
        if heled_ball is None:
            for ball in balls:
            ...
            draw_line_for_catapulted_ball(
                original_x, original_y, mouse_x, mouse_y, color
            )
    elif (
        not pygame.mouse.get_pressed()[2]
        and keys[pygame.K_LCTRL]
        and heled_ball is not None
        and original_x
        and original_y
        and not (help_menu_pressed or info_menu_pressed)
    ):
        if len(balls) > 0 and heled_ball is not None:
            new_vector_x, new_vector_y = calc_vector(
                original_x, original_y, mouse_x, mouse_y, flip=True
            )
            balls[heled_ball]["vector"]["x"], balls[heled_ball]["vector"]["y"] = (
                new_vector_x * 0.2,
                new_vector_y * 0.2,
            )
            original_x, original_y = None, None
            ...
            heled_ball = None
    # Drag and drop balls
    elif pygame.mouse.get_pressed()[2] and not (
        help_menu_pressed or info_menu_pressed
    ):  # Right mouse button
        mouse_x, mouse_y = pygame.mouse.get_pos()
        if heled_ball is None:
            ...
            balls[heled_ball]["x"] = mouse_x
            balls[heled_ball]["y"] = mouse_y
    elif not pygame.mouse.get_pressed()[2] and not (
        help_menu_pressed or info_menu_pressed
    ):
        if len(balls) > 0 and heled_ball is not None:
            balls[heled_ball]["vector"]["x"], balls[heled_ball]["vector"]["y"] = (
                calc_vector(mouse_pos[-3][0], mouse_pos[-3][1], mouse_x, mouse_y)
            )
            balls[heled_ball]["is_held"] = False
            heled_ball = None
    ...
    space_pressed = True
    if keys[pygame.K_SPACE] or help_menu_pressed or info_menu_pressed:
        space_pressed = False
    if space_pressed:
        simulate_physics()
